...
- `TIMEOUT_DAYS=1`: duração da punição (o Discord aceita no máximo 28 dias). O SAS envia uma DM laranja com a prova após aplicar o timeout por hacking e informa que ele é preventivo e que reincidências podem causar punições maiores. Uma DM verde é enviada quando o timeout expira ou é removido. Timeouts manuais geram uma DM laranja sem revelar o moderador e sem prova; o canal de logs recebe o moderador e uma prova visual reconstruída com as últimas 20 mensagens acessíveis do usuário. Usuários podem bloquear DMs do servidor.
